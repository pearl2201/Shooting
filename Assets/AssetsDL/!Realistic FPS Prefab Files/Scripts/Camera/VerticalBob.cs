			...
			if (timer > Mathf.PI * 2.0f){
				...
				if(!FPSWalkerComponent.climbing){
					...
					if (FPSWalkerComponent.grounded && (FPSWalkerComponent.landStartTime + 0.4f) < Time.time){
						GetComponent<AudioSource>().clip = walkSounds[Random.Range(0, walkSounds.Length)];//Select a random footstep sound from walkSounds array
						GetComponent<AudioSource>().volume = 0.5f;
						GetComponent<AudioSource>().Play();
					}
				}else{
					GetComponent<AudioSource>().clip = climbSounds[Random.Range(0, climbSounds.Length)];//Select a random climb sound from climbSounds array
