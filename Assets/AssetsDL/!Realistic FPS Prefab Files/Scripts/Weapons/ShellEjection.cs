...
public class ShellEjection : MonoBehaviour
{
	...
	void FixedUpdate(){
		...
		if(startTime + Time.fixedDeltaTime > Time.time){
			...
			GetComponent<Rigidbody>().AddRelativeTorque(Vector3.up * (Random.Range (rotateAmt * 1.75f,rotateAmt) * shellRotateSide));
			GetComponent<Rigidbody>().AddRelativeTorque(Vector3.right * (Random.Range (rotateAmt * 4,rotateAmt * 6) * shellRotateUp));
		}
		...
		if(FPSMainTransform == FPSWalkerComponent.initialParent){//if player is not on a moving platform
			...
			&& FPSWalkerComponent.grounded){
				Vector3 tempVelocity = playerObj.transform.GetComponent<Rigidbody>().velocity;
				tempVelocity.y = 0.0f;
			...
			if(startTime + 0.5f < Time.time && parentState){
				...
				myTransform.parent = null;
				//add player velocity to shell when unparenting from player object to prevent shell from suddenly changing direction
				GetComponent<Rigidbody>().AddForce(FPSMainTransform.GetComponent<Rigidbody>().velocity, ForceMode.VelocityChange);	
			}		
		}	
	}
