...
public class WeaponBehavior : MonoBehaviour {
				...
				&& (Mathf.Abs(horizontal) > 0 || Mathf.Abs(vertical) > 0)){
			    	if(IronsightsComponent.reloading){
						...
						if(bulletsToReload != bulletsPerClip){
							...
							weaponMesh.GetComponent<Animation>()["Neutral"].speed = 1.5f;
							weaponMesh.GetComponent<Animation>().Play("Neutral", PlayMode.StopAll);//play reloading animation	
						}	
				...
				||FPSWalkerComponent.cancelSprint){
					if(sprintAnimState){//animate weapon up
						...
						if(!PistolSprintAnim){
							...
							GetComponent<Animation>()["RifleSprinting"].speed = -1.4f;
							...
							//reverse animation speed for smooth changing of direction/reversal
							//animation will need to finish before recoveryTime has elapsed to prevent twisting of view when recovering from sprint
							GetComponent<Animation>()["PistolSprinting"].speed = -1.75f;
							GetComponent<Animation>().CrossFade("PistolSprinting", 2.25f,PlayMode.StopAll);	
						}
					...
					if(!sprintAnimState){//animate weapon down
						...
						if(PlayerWeaponsComponent.switching && IronsightsComponent.reloading){
							weaponMesh.GetComponent<Animation>().CrossFade("Reload", 0.35f,PlayMode.StopAll);//play firing animation
						}
					}
				}
			}
	...
	IEnumerator SpawnShell (){
		...
		if(shell.GetComponent<Rigidbody>()){
			shell.GetComponent<Rigidbody>().AddForce((transform.TransformDirection(shellEjectDirection) * shellForce / Time.timeScale), ForceMode.Impulse);
		}
	   	...
	   	if (canShoot){
			...
			if(meleeSwingDelay == 0){//if this is not a melee weapon
				...
				if(swingSide){//determine which side to swing melee weapon
					...
					weaponMesh.GetComponent<Animation>().Play("MeleeSwingRight",PlayMode.StopAll);//play weapon swing animation
					...
					weaponMesh.GetComponent<Animation>().Play("MeleeSwingLeft",PlayMode.StopAll);//play weapon swing animation
