	...
	public IEnumerator SelectWeapon ( int index  ){
		...
		if(!weaponOrder[childNum].GetComponent<WeaponBehavior>().PistolSprintAnim){
			...
			if(!FPSWalkerComponent.canRun){
				weaponOrder[childNum].GetComponent<Animation>()["RifleSprinting"].normalizedTime = 0;
				weaponOrder[childNum].GetComponent<Animation>()["RifleSprinting"].speed = 1.5f;
				weaponOrder[childNum].GetComponent<Animation>().CrossFade("RifleSprinting", 0.00025f,PlayMode.StopAll);
			}else{
				//if player is sprinting, keep weapon in sprinting position during weapon switch
				weaponOrder[childNum].GetComponent<Animation>()["RifleSprinting"].normalizedTime = 1;
			}
		}else{//weapon uses pistol sprinting animation
			//animate previous weapon down
			if(!FPSWalkerComponent.canRun){
				weaponOrder[childNum].GetComponent<Animation>()["PistolSprinting"].normalizedTime = 0;
				weaponOrder[childNum].GetComponent<Animation>()["PistolSprinting"].speed = 1.5f;
				weaponOrder[childNum].GetComponent<Animation>().CrossFade("PistolSprinting", 0.00025f,PlayMode.StopAll);
			}else{
				//if player is sprinting, keep weapon in sprinting position during weapon switch
				weaponOrder[childNum].GetComponent<Animation>()["PistolSprinting"].normalizedTime = 1;
			}
		}
